...
file_handler1 = logging.FileHandler('output.log', mode= 'w')
file_handler1.setFormatter(logging.Formatter('%(message)s'))
logger.addHandler(file_handler1)
file_handler2 = logging.FileHandler('hyperparameter_tune.log')
...
def read_and_prepare_data(validation_mode, k=5, augment=True):
    ...
    if validation_mode == 'k_fold':
        ...
        logger.info(f"Dimension of the dataset before Augmentaion = {len(images)} images")
        ...
        new_no_of_images = 0
        if AUGMENT:
            for i in folded_augmented_images:
                new_no_of_images += len(i)
            logger.info(f"Total number of images after Augmentation = {len(images)+new_no_of_images}")
        logger.info(f" Out of which {len(folded_images[0])} are used for testing and {len(images)+new_no_of_images-len(folded_images[0])}")
...
def train_model(model, train_images, train_labels, test_images, test_labels, show_performance_by_epoch=False):
    ...
    logger2.info(f"Number of epochs = {epochs}, Batch size in each epochs = {batch_size}")
...
def generate_predictions(model, test_images, test_labels):
    ...
    # Run validation data through model and logger.info mean absolute error
    ...
    for i in range(len(raw_predictions)):
        ...
        logger.info(f"prediction: {raw_predictions[i]} and diff: {abs_error}")
...
def show_validation_results(predictions, show_plots=True, print_error=True):
    ...
    if print_error:
        ...
        logger.info('Root Mean Square Error: ' + str(round(float(rmse), 2)) + ' knots')
        logger2.info('Root Mean Square Error: ' + str(round(float(rmse), 2)) + ' knots')
    if show_plots:
        ...
        logger.info('Graph of median absolute error by category saved as median_abs_error_by_category.png')
        plt.clf()
        ...
        logger.info('Graph of error distribution by category saved as error_dist_by_category.png')
...
def print_progress(action, progress, total):
    ...
    logger.info('\r' + action + '... ' + str(percent_progress) + '% (' + str(progress) + ' of ' + str(total) + ')')
...
def predict_image():
    ...
    logger.info(model.predict(images))
    '''
    ...
    for x in range(10):
        ...
        logger.info(f"labels{x}: {labels[x]}")
    ...
    for i in range(len(image)):
        logger.info(f"images{i}: {image[i]}")
...
if __name__ == "__main__":
    ...
    NUM_FOLDS = 2
    logger2.info(f"For k = {NUM_FOLDS} Folds, Augmentation = {str(AUGMENT)}, ")
    ...
    for i in range(NUM_FOLDS):
        ...
        logger.info(test_images[i].shape)
