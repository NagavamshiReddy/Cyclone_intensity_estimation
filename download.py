...
def download_zips(years):
    ...
    for year in years:
        ...
        year_url = 'https://www.ncei.noaa.gov/data/hurricane-satellite-hursat-b1/archive/v06/' + year
        year_html_page = requests.get(year_url).text
        parsed_page = BeautifulSoup(year_html_page, 'html.parser')
        year_zipfile_urls = [year_url + '/' + node.get('href') for node in
                                    parsed_page.find_all('a') if node.get('href').endswith('tar.gz')]
        print('\n' + year + ' file loaded.')

        files_processed = 0
        for zipfile_url in year_zipfile_urls:
            ...
            storm_name = zipfile_url.split('_')[-2]
            year = int(zipfile_url.split('_')[3][:4])
            ...
            if file_has_match_in_best_track:
                ...
                request = requests.get(zipfile_url, allow_redirects=True)
            ...
            files_processed += 1
            print_progress('Processing Files for ' + str(year), files_processed, len(year_zipfile_urls))
